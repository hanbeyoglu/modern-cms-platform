...
export const STORE_SOCIAL_PLATFORM_LABELS: Record<StoreSocialPlatform, string> = {
  INSTAGRAM: 'Instagram',
  FACEBOOK: 'Facebook',
  X: 'X / Twitter',
  TIKTOK: 'TikTok',
...
export function validateStoreSocialLinks(links: StoreSocialLink[]): string | null {
  ...
  for (const link of links) {
    ...
    if (link.platform !== 'OTHER') {
      if (seen.has(link.platform)) return `${STORE_SOCIAL_PLATFORM_LABELS[link.platform]} yalnızca bir kez eklenebilir`;
