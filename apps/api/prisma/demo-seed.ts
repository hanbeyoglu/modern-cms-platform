...
async function seedMall(demo: DemoMall, createdBy: string) {
  ...
  for (let index = 0; index < campaignRows.length; index++) {
    ...
    await prisma.campaign.upsert({
      ...
      update: {
        ...
        description,
        sharedCoverImageId: campaignCover.id,
        publishStartAt: daysFromNow(startOffset > 0 ? startOffset : -3),
        publishEndAt: null,
        campaignStartAt: daysFromNow(startOffset),
        campaignEndAt: daysFromNow(endOffset),
  ...
  for (let index = 0; index < eventRows.length; index++) {
    ...
    await prisma.event.upsert({
      ...
      update: {
        ...
        shortDescription: description,
        description,
        sharedCoverImageId: eventCover.id,
        publishStartAt: daysFromNow(-2),
        publishEndAt: null,
        eventStartAt: daysFromNow(startOffset),
        eventEndAt: daysFromNow(endOffset),
        status: 'PUBLISHED',
  ...
  await prisma.sliderItem.upsert({
    ...
    update: {
      sharedImageId: hero.id,
      sharedMobileImageId: hero.id,
