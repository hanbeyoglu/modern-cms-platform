...
export class GlobalStoresService {
  ...
  async findDuplicateByName(name: string, excludeId?: string): Promise<GlobalStoreResponse | null> {
    const normalizedName = normalizeStoreName(name);
    return this.prisma.globalStore.findFirst({
      where: {
        normalizedName,
        deletedAt: null,
  ...
  async create(dto: CreateGlobalStoreDto, user: User): Promise<GlobalStoreResponse> {
    ...
    const duplicate = await this.findDuplicateByName(dto.name);
    if (duplicate) {
      throw new ConflictException('Bu isimde bir global mağaza zaten var.');
    }

    const socialLinks = this.resolveSocialLinks(dto.socialLinks);
    ...
    const row = await this.prisma.globalStore.create({
      data: {
        ...
        socialLinksJson: socialLinks.length > 0 ? (socialLinks as Prisma.InputJsonValue) : undefined,
  ...
  async update(id: string, dto: UpdateGlobalStoreDto, user: User): Promise<GlobalStoreResponse> {
    ...
    if (dto.name !== undefined) {
      const duplicate = await this.findDuplicateByName(dto.name, existing.id);
      if (duplicate) {
        throw new ConflictException('Bu isimde bir global mağaza zaten var.');
      }
    }
    ...
    const socialLinks =
      dto.socialLinks !== undefined ? this.resolveSocialLinks(dto.socialLinks ?? []) : undefined;
    ...
    const data: Prisma.GlobalStoreUpdateInput = {
      ...
      ...(socialLinks !== undefined && {
        socialLinksJson:
          socialLinks.length === 0 ? Prisma.JsonNull : (socialLinks as Prisma.InputJsonValue),
      }),
  ...
  private resolveSocialLinks(raw: StoreSocialLink[] | null | undefined): StoreSocialLink[] {
    const links = parseStoreSocialLinks(raw);
    const err = validateStoreSocialLinks(links);
